        ...
        <nav className={`relative inline-flex items-center h-16 rounded-lg bg-card text-foreground border px-2 ${className}`}>
            {items.map(({ id, icon, label, onClick }, index) => (
                <a
                    ...
                    className={`relative z-20 flex h-full cursor-pointer items-center justify-center p-5 space-x-2 ${iconContainerClassName}`}
                    onClick={() => handleItemClick(index, onClick)}
                    aria-label={label}
                >
                    {cloneElement(icon, {
                        className: `w-5 h-5 transition-opacity duration-100 ease-in-out ${activeIndex === index ? 'opacity-100' : 'opacity-40'
                            } ${icon.props.className || ''} ${iconClassName || ''}`,
                    })}
                    {label && (
                        <span className={`font-medium transition-opacity duration-100 ease-in-out ${activeIndex === index ? 'opacity-100 text-primary' : 'opacity-40'}`}>
                            {label}
                        </span>
                    )}
                </a>
